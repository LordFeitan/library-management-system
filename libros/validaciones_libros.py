...
def validar_id_libro(id_libro):
    ...
    if not id_libro.isalnum():
        raise ValueError(
            'El ID solo puede contener caracteres alfanuméricos (sin espacios ni símbolos).')
...
def validar_titulo(titulo):
    ...
    if not re.match(patron, titulo):
        raise ValueError(
            'El título contiene caracteres no permitidos. Use solo letras, números, espacios y puntuación básica.')
...
def validar_autor(autor):
    ...
    if len(nombre) > 100:
        raise ValueError(
            'El nombre del autor no puede exceder 100 caracteres.')

    patron = r'^[a-záéíóúüñA-ZÁÉÍÓÚÜÑ\s.]+$'
    if not re.match(patron, nombre):
        raise ValueError(
            'El nombre del autor solo puede contener letras, espacios, acentos y puntos.')
    ...
    if hasattr(autor, 'nacionalidad') and autor.nacionalidad:
        ...
        if not re.match(patron_nac, nacionalidad):
            raise ValueError(
                'La nacionalidad solo puede contener letras, espacios y acentos.')
...
def validar_categoria(categoria_id):
    ...
    if len(categoria_id) != 6:
        raise ValueError(
            'El ID de categoría debe tener el formato CATXXX (ej: CAT001).')
    ...
    if not any(cat['id_categoria'] == categoria_id for cat in categorias):
        raise ValueError(
            f'El ID de categoría "{categoria_id}" no existe en el catálogo.')
