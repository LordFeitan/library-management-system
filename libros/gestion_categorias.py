...
def modificar_categoria(id_categoria, nuevo_nombre=None, nueva_descripcion=None):
	...
	for cat in categorias:
		if cat['id_categoria'] == id_categoria:
			...
			# Si se proporciona nuevo nombre, verificar que no exista
	...
	if not categoria_encontrada:
		raise ValueError(f"No se encontró la categoría con ID '{id_categoria}'.")
...
def eliminar_categoria(id_categoria):
	...
	# Verificar si hay libros usando esta categoría (por ID)
	libros = listar_libros()
	categoria = buscar_categoria_por_id(id_categoria)
	
	if categoria:
		# Buscar libros que tengan este ID de categoría
		libros_con_categoria = [l for l in libros if l.categoria == id_categoria]
		if libros_con_categoria:
			raise ValueError(f"No se puede eliminar la categoría porque hay {len(libros_con_categoria)} libro(s) asociado(s).")
	
	categorias = cargar_datos(RUTA_JSON_CATEGORIAS)
	categorias_filtradas = [cat for cat in categorias if cat['id_categoria'] != id_categoria]
...
def mostrar_menu_seleccion_categoria():
	...
	while True:
		try:
			...
			# Seleccionar categoría existente - RETORNA ID
			...
			# Agregar nueva categoría
				...
				try:
					...
					print(f"✅ Categoría '{nueva_cat['nombre']}' agregada correctamente.")
