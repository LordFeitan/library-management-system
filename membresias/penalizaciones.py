...
class PenalizacionMonetaria(Penalizacion):
    ...
    def aplicar_penalizacion(self):
        ...
        print(
            f"⚠️  Penalización aplicada: ${monto:.2f} por {self._dias_retraso} días de retraso")
        return monto
...
class PenalizacionSuspension(Penalizacion):
    ...
    def aplicar_penalizacion(self):
        ...
        print(
            f" Miembro suspendido por {self._dias_suspension} días. Hasta: {fecha_fin_suspension}")
...
class PenalizacionCombinada(Penalizacion):
    ...
    def __init__(self, prestamo, dias_retraso: int):
        super().__init__(prestamo, dias_retraso)
        self._penalizacion_monetaria = PenalizacionMonetaria(
            prestamo, dias_retraso)
        self._penalizacion_suspension = PenalizacionSuspension(
            prestamo, dias_retraso)
