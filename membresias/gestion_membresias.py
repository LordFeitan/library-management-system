...
def listar_prestamos():
    ...
    for miembro in miembros:
        for prestamo in miembro.get('prestamos', []):
            ...
            prestamo_copia['miembro'] = {
                'id_miembro': miembro['id_miembro'], 'nombre': miembro['nombre']}
...
def registrar_prestamo(id_prestamo, id_miembro, id_libro):
    ...
    if stock_actual <= 0:
        raise LibroNoDisponibleError(
            f'No hay stock disponible para el libro {libro.get("titulo", id_libro)}.')

    miembro = next(
        (m for m in miembros if m['id_miembro'] == id_miembro), None)
    if not miembro:
        raise MiembroNoEncontradoError(
            f'Miembro con ID {id_miembro} no encontrado.')

    if any(p.get('id_prestamo') == id_prestamo for m in miembros for p in m.get('prestamos', [])):
        raise PrestamoActivoError(
            f'Ya existe un préstamo con ID {id_prestamo}.')
    ...
    for m in miembros:
        for prestamo_data in m.get('prestamos', []):
            if prestamo_data['id_prestamo'] == id_prestamo:
                fecha_prestamo = date.fromisoformat(
                ...
                if prestamo_obj.esta_retrasado:
                    penalizacion = PenalizacionCombinada(
                        prestamo_obj, prestamo_obj.dias_retraso)
                    resultado = penalizacion.aplicar_penalizacion()
                    print(f"🚨 PENALIZACIÓN APLICADA:")
                    print(f"   - Multa: ${resultado['multa']:.2f}")
                    print(
                        f"   - Suspensión: {resultado['dias_suspension']} días")
...
def agregar_miembro(miembro: Miembro):
    ...
    if any(m['id_miembro'] == miembro.id_miembro for m in miembros):
        raise IDDuplicadoError(
            f'Ya existe un miembro con el ID {miembro.id_miembro}.')
...
def eliminar_miembro(id_miembro):
    ...
    miembros_nuevos = [
        m for m in miembros if m['id_miembro'] != id_normalizado]
