...
def validar_nombre(nombre):
    ...
    if not re.match(patron, nombre):
        raise ValueError(
            'El nombre solo puede contener letras, espacios y acentos. No se permiten números ni caracteres especiales.')
...
def validar_correo(correo):
    ...
    if not re.match(patron, correo):
        raise ValueError(
            'El correo no tiene un formato válido. Ejemplo válido: usuario@dominio.com')

    if len(correo) > 254:  # RFC 5321
        raise ValueError(
            'El correo es demasiado largo (máximo 254 caracteres).')
...
def validar_telefono(telefono):
    ...
    if not telefono_limpio.isdigit():
        raise ValueError(
            'El teléfono solo puede contener números (se permiten espacios y guiones como separadores).')
